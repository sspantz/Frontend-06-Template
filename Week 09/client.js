...
class ResponseParser {
  ...
  receiveChar(c) {
    if (this.status === this.WAITING_STATUS_LINE) {
      if (c === "\r") this.status = this.WAITING_STATUS_LINE_END;
      ...
      if (c === ":") this.status = this.WAITING_HEADER_SPACE;
      else if (c === "\r") {
        this.status = this.WAITING_HEADER_LINE_BLOCK_END;
        this.bodyParser = new TrunckedBodyParser();
      } else this.headerName += c;
    } else if (this.status === this.WAITING_HEADER_SPACE) {
      if (c === " ") this.status = this.WAITING_HEADER_VALUE;
    } else if (this.status === this.WAITING_HEADER_VALUE) {
      if (c === "\r") {
        this.status = this.WAITING_HEADER_VALUE_END;
        this.headers[this.headerName] = this.headerValue;
        this.headerName = this.headerValue = "";
      } else this.headerValue += c;
    } else if (this.status === this.WAITING_HEADER_VALUE_END) {
      if (c === "\n") this.status = this.WAITING_HEADER_NAME;
    } else if (this.status === this.WAITING_HEADER_LINE_BLOCK_END) {
...
class TrunckedBodyParser {
  constructor() {
    ...
    this.status = this.WAITING_LENGTH;
  }
  receive(c) {
    if (this.status === this.WAITING_LENGTH) {
      if (c === "\r") {
        if (this.length === 0) this.isFinised = true;
        this.status = this.WAITING_LENGTH_END;
      } else {
        this.length *= 16;
        this.length += parseInt(c, 16);
      }
    } else if (this.status === this.WAITING_LENGTH_END) {
